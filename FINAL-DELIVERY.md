...
界面里有 **“工作”** 时优先使用“工作”模式；没有就使用普通 ChatGPT 对话。桌面版如果提供 Codex，也可以让 Codex 负责代码检查和故障修复。

最低使用 GPT-5.6 Sol，思考程度选择“高”。遇到部署失败、资源绑定、数据库迁移、版本升级或账号串号时改成“超高”。

把 `deployment/DEPLOY-PROMPT.txt` 里的全部文字复制给 GPT。

GPT 会先确认当前实际操作浏览器里的 GitHub 和 Cloudflare 登录身份，再带你打开官方部署页面。如果使用“工作”模式里的独立浏览器或云端浏览器，也要检查那个浏览器自己的登录账号。
