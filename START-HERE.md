...
- 推荐模式：界面里有 **“工作”** 时优先使用“工作”模式；没有就用普通 ChatGPT 对话，不需要卡在这里。
- 桌面版如果提供 Codex，也可以让 Codex 负责代码检查和故障修复。
- 最低模型：GPT-5.6 Sol
- 默认思考程度：高
...
如果使用“工作”模式里的独立浏览器或云端浏览器，要注意它有自己的登录状态，不一定等于你电脑 Chrome 里已经登录的 GitHub / Cloudflare 账号。

## 3. GPT 会先检查账号

GPT 必须先确认当前实际操作浏览器登录的是谁的 GitHub、谁的 Cloudflare，以及这是这个 Cloudflare 账号里的第几个作品集网站。
