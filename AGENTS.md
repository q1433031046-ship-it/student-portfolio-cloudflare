...
3. Before mutation, the bridge verifies `deployment/workers-builds-upgrade-bridge.json`, the frozen v1.3.0 product payload and build output, fixed local ids, and every active-version DB/KV/R2/remote-var/Secret-binding fingerprint.
...
6. Re-read the same existing Worker immediately before deploying. Deploy with a process-unique temporary config that contains no source `vars`, plus `--keep-vars` and `--strict`; always delete the temporary file. This preserves dashboard vars, Secrets, fixed resources, and the existing workers.dev configuration.
7. Report `[BRIDGE][SUCCESS]` only after observing a new Worker version and exactly matching the pre-deploy resource fingerprint. `[BRIDGE][CHECK]` is progress only; `[BRIDGE][BLOCKED]` and `[BRIDGE][FAILED]` are never success.
