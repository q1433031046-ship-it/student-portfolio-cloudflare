...
The current release contract is version `1.3.0`, source ref `refs/tags/v1.3.0`, commit `4658bc834d6ea21aa94ce0db0d9c99e82b856235`. Before showing or executing an upgrade prompt, verify the prompt's exact UTF-8 SHA-256 against both release manifests. `main` may advertise a later version manifest; the prompt itself must come from that manifest's matching `v<version>` tag.
...
7. This bridge Candidate is existing-site-only and must not provision a new site. New-site setup continues from the immutable released template through its separate `npm run cloudflare:deploy:new` path; do not reuse this Candidate's `npm run deploy` bridge to create a Worker or storage resource.
...
## Workers Builds existing-site upgrade bridge

This section applies only to the audited `v1.3.0` bridge Candidate for an existing Worker already connected to its own student repository. It does not authorize a new Worker, D1 database, KV namespace, R2 bucket, merge, tag, or release.

1. The chat-side agent changes GitHub only. Preserve the existing site's `wrangler.jsonc`, including the exact Worker configuration, fixed `DB.database_id`, fixed `MEDIA_KV.id`, optional existing `BUCKET`, `workers_dev`, and vars. Never request a Cloudflare Token, Cookie, administrator password, deployment code, or recovery code.
2. Cloudflare Workers Builds must run `npm run build` and then `npm run deploy`. Its selected build token must grant Account `Workers Scripts:Edit` and Account `D1:Edit`; the token stays in Cloudflare and is never requested in chat or committed. `WRANGLER_CI_OVERRIDE_NAME` is mandatory and is the only Worker name for every remote operation. Missing or invalid identity and a missing Worker are blocked; the bridge never creates or adopts a resource.
3. Before mutation, the bridge verifies `deployment/workers-builds-upgrade-bridge.json`, the frozen v1.3.0 product payload and build output, fixed local ids, and the complete binding inventory of every active version. Only `ASSETS`, the single fixed `DB`, the single fixed `MEDIA_KV`, optional `BUCKET`, remote vars, and Secret names are supported; any other local or live resource binding blocks before migration.
4. The only accepted pending sets are empty, `0007_legacy_media_and_access_state.sql`, or `0006_auth_v2.sql` followed by `0007_legacy_media_and_access_state.sql`. A list failure, ambiguous output, duplicate, reorder, earlier or unknown migration, or SHA-256 mismatch blocks before apply.
5. Apply pending migrations once, then list again and require an empty set. Every apply or re-list failure stops before Worker deployment, including missing `D1:Edit`; the manual runtime-bootstrap permission fallback is forbidden in this bridge.
6. Re-read the same existing Worker immediately before deploying. Deploy with a process-unique temporary config that contains no source `vars`, plus `--keep-vars`, `--strict`, `--experimental-provision=false`, `--experimental-auto-create=false`, and `--autoconfig=false`; always delete the temporary file. Wrangler automatic resource provisioning and draft resource creation must remain explicitly disabled.
7. Report `[BRIDGE][SUCCESS]` only after observing a new Worker version and exactly matching the complete pre-deploy binding fingerprint. Never echo raw Wrangler apply/deploy output or resource identifiers. `[BRIDGE][CHECK]` is progress only; `[BRIDGE][BLOCKED]` and `[BRIDGE][FAILED]` are never success.
8. Different student repositories remain independent: each supplies its own fixed ids, Worker override, build authorization, and temporary files. No global resource name or id is shared by the bridge.
