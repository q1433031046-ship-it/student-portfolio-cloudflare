...
export async function POST(request: Request) {
  try {
    ...
    if (!isRecord(body) || typeof body.password !== "string") {
      return Response.json({ error: "请输入管理员密码", code: "PASSWORD_MISSING" }, { status: 400, headers: { "Cache-Control": "no-store" } });
    }
    ...
    return Response.json({ error: "登录暂时不可用", code: "ADMIN_LOGIN_UNAVAILABLE" }, { status: 500, headers: { "Cache-Control": "no-store" } });
  }
}
