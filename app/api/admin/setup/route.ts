...
export async function GET(request: Request) {
  try {
    if (!isSitesAuthPlatform()) {
      const localState = await getLocalAdministratorState();
      if (localState.state === "initial_setup") {
        return Response.json({ ...localState, identity: null }, { headers: noCacheHeaders });
      }
      if (localState.state === "password_reset_required") {
        return Response.json({ ...localState, identity: null }, { headers: noCacheHeaders });
      }
    }

    const identity = await authorizeAdmin(request);
    if (!identity) return Response.json({ error: "请先输入管理员密码", code: "ADMIN_LOGIN_REQUIRED" }, { status: 401, headers: noCacheHeaders });
    if (identity.kind === "token") return Response.json({ error: "服务令牌不能进入初始化流程", code: "SERVICE_TOKEN_NOT_ALLOWED" }, { status: 403, headers: noCacheHeaders });
    ...
    return Response.json({ error: "管理员状态暂时无法读取", code: "ADMIN_STATE_UNAVAILABLE" }, { status: 503, headers: noCacheHeaders });
  }
}
...
export async function POST(request: Request) {
  if (isSitesAuthPlatform()) {
    ...
    if (!identity || identity.kind === "token") {
      return Response.json({ error: "请先完成网站登录", code: "ADMIN_LOGIN_REQUIRED" }, { status: 401, headers: noCacheHeaders });
    }
    try {
      await bindSiteOwner(identity);
      return Response.json({ state: "ready", identity: identity.user }, { headers: noCacheHeaders });
    } catch (error) {
      return Response.json({ error: errorMessage(error), code: "SITE_OWNER_BIND_FAILED" }, { status: 400, headers: noCacheHeaders });
    }
  }
  ...
  try {
    ...
    if (!isRecord(body) || typeof body.initialCode !== "string" || typeof body.password !== "string") {
      return Response.json({ error: "请填写一次性部署口令和管理员密码", code: "SETUP_INPUT_MISSING" }, { status: 400, headers: noCacheHeaders });
    }
    ...
    if (error instanceof AuthError || isRequestBodyError(error)) {
      ...
      }, { status: error.status, headers: noCacheHeaders });
    }
    console.error(JSON.stringify({ message: "local administrator setup failed", error: errorMessage(error) }));
    return Response.json({ error: "管理员初始化暂时无法完成", code: "ADMIN_SETUP_FAILED" }, { status: 500, headers: noCacheHeaders });
  }
}
