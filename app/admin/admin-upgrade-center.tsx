    ...
    <section id="program-upgrade-center" data-native-upgrade-center tabIndex={-1} aria-labelledby="program-upgrade-title">
      ...
      <div className="grid">
        ...
        <div>
          ...
          <small>内容保留；恢复码确认按实际版本条件处理，同 C 部署不自动轮换。</small>
        </div>
      </div>
      <p className="note">
        {IS_UPGRADE_PREPARATION ? "当前 C 尚未正式分发，复制的是完整升级准备指令。先核对正式标签、清单、准确审核及原站地址适配；条件不齐只读盘点并停止升级写入。" : "先核对固定发布标签与配套摘要、原站资源和内容，再准备准确同站升级对象。"}
      </p>
      <div className="actions">
        ...
        <details>
          ...
          <div className="detail">
            ...
            <p><strong>原站适配：</strong>当前源码的 Pages、上传页和 ZIP 后台入口含本站专用地址。准确产物未适配自己的原站时停止；不要复制其他网站账号、资源 ID 或凭据。保留原 Worker、DB、MEDIA_KV、必要旧绑定、免费套餐与内容。</p>
            <p><strong>必要迁移：</strong>C 涉及 0008–0011，按真实账本与摘要只处理缺失增量；不能沿用旧“止于 0007”说明或靠打开 /admin 补齐。cloudflare:deploy 会真实迁移和部署，不是只读工具。</p>
            <p><strong>恢复码条件：</strong>已确认的 1.3.0 到 C 需要本人用当前恢复码确认、重设密码并保存新码；已确认的 1.3.1-b 到 C 为精确例外，同 C 部署不自动轮换。新文件名按实际版本生成，下载后核对站点和版本；秘密不发到聊天。</p>
            <p><strong>历史说明：</strong>deployment/template-version.json 与 upgrade-prompt.json 的 v1.3.0 发布记录原样保留，不能当成 C 正式分发凭证。当前完整准备文本由两个复制入口共享。</p>
