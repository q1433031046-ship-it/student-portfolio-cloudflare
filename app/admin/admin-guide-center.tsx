        ...
        <div className="content">
          ...
          <section className="guide" id="admin-guide-password">
            ...
            <ul>
              ...
              <li>从已确认的 1.3.0 到 C 需要当前恢复码确认并重设密码；已确认的 1.3.1-b 到 C 为精确例外，同 C 部署不自动轮换。</li>
          ...
          <section className="guide" id="admin-guide-upgrade">
            ...
            <p>{IS_UPGRADE_PREPARATION ? "当前 C 仍为未正式分发的候选。下方提供完整升级准备指令；本站在线不表示其他网站已可直接升级。" : "先核对固定发布标签、准确提交与配套清单，再按已校验指令准备同站升级。"}</p>
            <div className="callout"><strong>正式目标与原站适配先齐备</strong><p>先核对固定标签、提交、清单、prompt 摘要和正式审核。当前源码含本站专用 Pages 地址、上传地址和 ZIP 后台入口；准确目标产物未适配自己的原站时停止升级写入，不能复制本站账号或资源。历史 v1.3.0 清单继续作为历史保留。</p></div>
            <div className="callout safe"><strong>恢复码由本人保管和输入</strong><p>从已确认的 1.3.0 到 C 需要当前恢复码确认、重设密码并保存新码；已确认的 1.3.1-b 到 C 是精确例外，同 C 部署不自动轮换。不要将密码或恢复码发送给 GPT。</p></div>
            <div className="callout"><strong>迁移与部署会真实写入</strong><p>先只读核对原 Worker、固定 DB、唯一 MEDIA_KV、必要旧 BUCKET 与资源指纹。C 的正常迁移涉及 0008–0011，只处理真实账本中缺少且结构与摘要匹配的增量；打开 /admin 不能代替这些迁移。cloudflare:deploy 会应用迁移并部署，不能当只读检查运行。</p></div>
            <h3>准备与执行分步确认</h3>
            <div className="steps">
