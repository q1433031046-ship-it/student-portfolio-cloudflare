      ...
      <div className="layout">
        ...
        <div className="content">
          ...
          <section className="guide" id="admin-guide-publish">
            ...
            <div className="flow"><div><b>1</b>保存草稿</div><div><b>2</b>冻结候选</div><div><b>3</b>核验 draft</div><div><b>4</b>提升同一 Deploy</div></div>
            <ul>
              <li>保存草稿更新后台版本，公开前台保持当前内容。</li>
              <li>点击“快速预览”时，快速预览会先自动保存当前修改，再打开管理员草稿版本。</li>
              <li>正式发布只生成一个 Netlify draft Deploy；从不可变地址核验完整制品后，提升同一个 Deploy。</li>
              <li>新建作品先填名称和分类、新建分类先填名称，保存草稿后再上传媒体。</li>
              <li>每次上传或替换图片后先保存草稿，再切换作品或后台栏目。</li>
              <li>左下角显示“有未保存修改”时，保存后再离开。</li>
              <li>发布前先检查 Netlify 用量；页面只显示媒体总量，不伪造实时剩余额度。</li>
              <li>失败时上一固定网站保持不变；“重新核验”和“重试原发布任务”不会创建第二个 Build。</li>
          ...
          <section className="guide" id="admin-guide-qr">
            ...
            <ul>
              <li>固定二维码只包含公开的 netlify.app URL，不含 token、Cookie 或后台权限。</li>
              <li>重新发布和失败重试不改变固定二维码；失败时仍打开上一成功版本。</li>
          ...
          <section className="guide" id="admin-guide-errors">
            ...
            <div className="grid">
              ...
              <article className="card"><h3>保存后前台仍是旧内容</h3><p>保存草稿不会更新固定站；进入“发布”，等待同一个 draft Deploy 核验和固定网址读回完成。</p></article>
              <article className="card"><h3>静态发布需要重新授权</h3><p>不要把 Token 或 Hook 发到聊天；由发布角色核对当前学生的 Netlify 账号和唯一 Site 后重新授权。</p></article>
            </div>
          </section>
