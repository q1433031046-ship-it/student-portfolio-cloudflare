...
import type { BackCoverSlide, HeroLayer } from "../portfolio/model";
...
export function BackCoverLayoutEditor({
  slide,
  customFontReady,
  onChange,
}: {
  ...
  onChange: (slide: BackCoverSlide) => void;
}) {
  ...
  function updateText(patch: Partial<Pick<BackCoverSlide, "eyebrow" | "title" | "statement" | "details">>) {
  ...
  function startPointer(event: ReactPointerEvent<HTMLElement>, layer: HeroLayer, mode: "move" | "resize") {
    ...
    const canvas = event.currentTarget.closest<HTMLElement>("[data-back-cover-layout-canvas]");
    if (!canvas) return;
  ...
  return (
    <div className={styles.layoutEditor} data-back-cover-editor>
      <div className={styles.previewDeviceSwitch} role="group" aria-label="封底预览设备">
        <button type="button" data-active={device === "desktop"} onClick={() => setDevice("desktop")}>桌面 16:9</button>
        <button type="button" data-active={device === "mobile"} onClick={() => setDevice("mobile")}>手机 4:5</button>
      </div>
      <div
        className={`${styles.layoutCanvas} ${styles.backCoverLayoutCanvas}`}
        data-layout-canvas
        data-back-cover-layout-canvas
        data-preview-device={device}
        style={{ aspectRatio: device === "mobile" ? 4 / 5 : 16 / 9 }}
      >
        {slide.media.src
        ...
        {slide.layers.filter((layer) => layer.visible).map((layer) => (
          ...
          >
            {layer.kind === "identity" && <>
              <InlineEditable tag="small" value={slide.eyebrow} ariaLabel="封底眉题" onCommit={(eyebrow) => updateText({ eyebrow })} />
              <InlineEditable tag="strong" value={slide.title} ariaLabel="封底标题" onCommit={(title) => updateText({ title })} />
            </>}
            {layer.kind === "statement" && <InlineEditable tag="span" value={slide.statement} ariaLabel="封底说明" onCommit={(statement) => updateText({ statement })} />}
            {layer.kind === "facts" && <InlineEditable tag="small" value={slide.details} ariaLabel="封底补充信息" onCommit={(details) => updateText({ details })} />}
            <i
      ...
      <p className={styles.layoutHint}>拖动文字改变位置，拖动右下角改变大小；双击文字可编辑，按 Enter 换行，Ctrl/⌘ + Enter 完成编辑。</p>
...
function InlineEditable({
  ...
  const props = {
    ...
    onBlur: (event: React.FocusEvent<HTMLElement>) => { setEditing(false); onCommit(normalizeEditableText(event.currentTarget.innerText)); },
